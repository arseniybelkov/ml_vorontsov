...
methods = {
    "regression": ["Линейная регрессия", "Перцептрон", "Надарая-Ватсона", "SVR"],
    "classification": [
...
task["method"] = np.random.choice(methods[task["type"]], size=3, replace=False).tolist()


print(task)
dt = date.today().strftime("%d/%m/%Y")
with open(f"generated_task_{dt}.txt".replace("/", "_"), "w") as f:
    f.write(str(task))
